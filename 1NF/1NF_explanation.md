...
Emmanuel described it like this: imagine Bonae unpacking a shopping list that says `eggs|milk|bread` — she would write **three separate lines**, one item each, instead of three items crammed into one cell. That is exactly what we did with every `|` column in the raw CSV.
...
|------|------|---------------|
| `project_worker_assignments_1nf.csv` | 15 | Main row minus the `|` columns |
