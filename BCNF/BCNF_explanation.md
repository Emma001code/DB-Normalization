...
3NF is good, but BCNF is stricter. The rule in plain words:

> **If column set X determines column Y, then X must be a candidate key — not just “part of” the key or some random column.**

We went through **every** 3NF table and asked: “Does anything determine something else without being the full primary key?”

**Good news:** 10 of our 12 tables were already fine.

**Two tables were not:** material prices and equipment rental costs included the **worker’s name** in the key, but the **price did not actually change** when the worker changed. We split those tables in two.
